...
class Serializer(object):
    """ table serialization mixin class """
    ...
    def serialize(self):
        ...
        keys = map.relationships.keys()
        for key in keys:
            value = getattr(self, key)
            if map.relationships[key].uselist:
                response[key] = self.serialize_list(value)
            else:
                # return the description for the display view
                # using naming convention for vocabulary tables
                try:
                    response[key] = getattr(value, key+"Desc")
                except:
                    pass
...
#         """ serialize id and desc for use as select element options
#         
#         Method assumes a naming convention where table "foolist" has primary
#         key column "fooID", sort description "fooDesc", and help text
#         "fooText".
#         """
#         map = inspect(self).mapper   
#         name = map.tables[0].name
#         root = re.sub("list\\b", "", name)
    
    @staticmethod
    def serialize_options(table):
        """ serialize a vocabulary table (e.g., maturitylist) as options
        """

        results = db.session.query(table).all()
        return [result._serialize_options() for result in results]
...
class Maturitylist(Base, Serializer):
...
try:
    ...
    LATEST_DISPOSITION_RESULTS = DISPOSITION_RESULTS
except:
    pass
...
FY_CHOICES = []
try:
    FY_RESULTS = Fiscalyears.query.order_by(desc("fiscalyearID")).all()
...
M_CHOICES = []
try:
    M_RESULTS = Months.query.order_by("monthID").all()
...
Q_CHOICES = []
try:
    Q_RESULTS = Quarters.query.order_by("quarterID").all()
...
Y_CHOICES = []
try:
    Y_RESULTS = Calendaryears.query.order_by(desc("calendaryearID")).all()
...
class Disposition(Base, Serializer):
    ...
    t_description = db.relationship("Description", backref="dispositions")
...
class Latest_disposition(Base, Serializer):
    ...
    startInM = Column(Integer, ForeignKey("months.monthID"),
                      info={"choices": M_CHOICES,
                            ...
                            "help": ""},
                      nullable=True, server_default=text("'0'"))
    startMChoice = db.relationship("Months", 
    ...
    finishInM = Column(Integer, ForeignKey("months.monthID"),
                       info={"choices": M_CHOICES,
                             ...
                             "help": ""},
                       nullable=True, server_default=text("'0'"))
    finishMChoice = db.relationship("Months", 
    ...
    t_description = db.relationship("Description", backref="disposition")
...
class Description(Base, Serializer):
    ...
    childs = association_proxy("child", "children")
    drivers = association_proxy("driverID", "vocabulary")
    stakeholders = association_proxy("stakeholderID", "vocabulary")
    
class Child(Base, Serializer):
    __tablename__ = "child"
    projectID = Column("projectID", Integer, 
                       ForeignKey("description.projectID"), 
                       primary_key=True)
    childID = Column("childID", Integer, 
                     ForeignKey("description.projectID"), 
                     primary_key=True)
    
    info={"label": "children",
          "attributeID": 200,
          "help": "For an absorbed project, enter the project ID of the " 
                  "surviving project. For a split project, enter the project "
                  "IDs of the child projects."}
    t_description = db.relationship("Description", 
                         primaryjoin=(projectID == Description.projectID),
                         backref=backref("childID",
                                         info=info,
                                         cascade="all, delete-orphan"))
    children = db.relationship(Description,
                               primaryjoin=(childID == Description.projectID),
                               backref="from_child")
...
class Driver(Base, Serializer):
    ...
    projectID = Column("projectID", Integer, 
                       ForeignKey("description.projectID"), 
                       primary_key=True)
    driverID = Column("driverID", Integer, 
                      ForeignKey("driverlist.driverID"), primary_key=True)
    
    info={"attributeID": 160,
          "label": "drivers",
          "help": "Identify the primary drivers, or rationale, for "
                  "this project."}
    t_description = relationship("Description", 
                                 backref=backref("driverID", 
                                                 info=info,
                                                 cascade="all, delete-orphan"))
    vocabulary = relationship("Driverlist")

class Stakeholder(Base, Serializer):
    __tablename__ = "stakeholder"
    projectID = Column("projectID", Integer, 
                       ForeignKey("description.projectID"),
                       nullable=False, primary_key=True, index=True, 
                       server_default=text("'0'"))
    stakeholderID = Column("stakeholderID", Integer, 
    ...
    info = {"attributeID": 130,
           "label": "stakeholders",
           "help": "Which organizations, besides the  sponsor stand to be "
                   "affected  (positively or negatively) by  this project?"}
    t_description = db.relationship("Description", 
                          backref=backref("stakeholderID",
                                          info=info,
                                          cascade="all, delete-orphan"))
    vocabulary = db.relationship("Stakeholderlist")
...
class Comment(Base, Serializer):
    ...
    t_description = db.relationship("Description", backref="comments")
...
class Portfolio(Base, Serializer):
    ...
    t_description = db.relationship("Description", backref="portfolio")
    strategys = association_proxy("strategyID", "vocabulary")

class Strategy(Base, Serializer):
    __tablename__ = "strategy"
    projectID = Column("projectID", Integer, 
                       ForeignKey("portfolio.projectID"),
                       primary_key=True, nullable=False, index=True, 
                       server_default=text("'0'"))
    strategyID = Column("strategyID", Integer, 
                        ForeignKey("strategylist.strategyID"),
                        primary_key=True, nullable=False, index=True,
                        server_default=text("'0'"))
    
    info={"attributeID": 220,
          "label": "strategies",
          "help": "Which Institute strategic goals does this project "
                  "support, if any?"}
    portfolio = db.relationship("Portfolio",  
                                backref=backref("strategyID",
                                                info=info,
                                                cascade="all, delete-orphan"))
    vocabulary = db.relationship("Strategylist")
...
class Project(Base, Serializer):
    ...
    t_description = db.relationship("Description", backref="project")
